...
async def test_search_bm25_returns_results():
    ...
    with patch("omnivore.api.routes.search.AsyncSessionLocal", _make_session_local(db)):
        resp = await search(request=_make_request(), body=SearchRequest(query="hello", mode="bm25"))
...
async def test_search_bm25_empty_results():
    ...
    with patch("omnivore.api.routes.search.AsyncSessionLocal", _make_session_local(db)):
        resp = await search(request=_make_request(), body=SearchRequest(query="zzz", mode="bm25"))
...
async def test_search_bm25_does_not_call_embed():
    ...
    with (
        patch("omnivore.api.routes.search.AsyncSessionLocal", _make_session_local(db)),
        patch("omnivore.api.routes.search.embed_texts", AsyncMock()) as mock_embed,
    ):
        await search(request=_make_request(), body=SearchRequest(query="q", mode="bm25"))
...
async def test_search_vector_no_redis_still_embeds():
    ...
    with (
        ...
        resp = await search(request=_make_request(arq_pool=None), body=SearchRequest(query="dense", mode="vector"))
...
async def test_search_hybrid_is_default_mode():
    ...
    with (
        ...
        resp = await search(request=_make_request(arq_pool=AsyncMock()), body=SearchRequest(query="test"))
...
async def test_search_result_fields_populated():
    ...
    with patch("omnivore.api.routes.search.AsyncSessionLocal", _make_session_local(db)):
        resp = await search(request=_make_request(), body=SearchRequest(query="q", mode="bm25"))
